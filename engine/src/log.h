...
namespace engine {
  ...
  class PUB_API Log {
    ...
    public:
     static void init() {
      Log::engine_log = std::shared_ptr<cpplog::Logger>(cpplog::create_log("ENGINE"));
      Log::engine_log->set_log_level(cpplog::Level::ERROR);
      Log::app_log = std::shared_ptr<cpplog::Logger>(cpplog::create_log("APP"));
      Log::app_log->set_log_level(cpplog::Level::ERROR);
     }
...
#define ENGINE_LOG_WARN(...) engine::Log::get_engine_log()->warning(__VA_ARGS__)
...
#define APP_LOG_WARNUNG(...) engine::Log::get_app_log()->warning(__VA_ARGS__)
#define EAPP_LOG_INFO(...) engine::Log::get_app_log()->info(__VA_ARGS__)
